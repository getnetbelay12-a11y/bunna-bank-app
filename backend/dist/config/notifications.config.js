...
const external_email_config_1 = require("./external-email.config");
const external_telegram_config_1 = require("./external-telegram.config");
...
exports.notificationsConfig = (0, config_1.registerAs)('notifications', () => {
    const externalEmail = (0, external_email_config_1.getExternalEmailSettings)();
    const externalTelegram = (0, external_telegram_config_1.getExternalTelegramSettings)();
    const emailSmtpHost = firstNonEmpty(process.env.EMAIL_SMTP_HOST, process.env.MAIL_HOST, externalEmail?.smtpHost);
    const emailSmtpUser = firstNonEmpty(process.env.EMAIL_SMTP_USER, process.env.MAIL_USER, externalEmail?.smtpUser);
    const emailSmtpPass = firstNonEmpty(process.env.EMAIL_SMTP_PASS, process.env.MAIL_PASSWORD, externalEmail?.smtpPass);
    const configuredEmailProvider = firstNonEmpty(process.env.EMAIL_PROVIDER, externalEmail?.provider);
    const resolvedEmailProvider = configuredEmailProvider === 'log' &&
        emailSmtpHost &&
        emailSmtpUser &&
        emailSmtpPass
        ? 'smtp'
    ...
    return {
        ...
        email: {
            ...
            sender: firstNonEmpty(process.env.EMAIL_SENDER, process.env.MAIL_FROM, externalEmail?.sender) ??
                'notifications@bunna-bank.local',
            endpoint: process.env.EMAIL_GENERIC_ENDPOINT ?? '',
            apiKey: process.env.EMAIL_GENERIC_API_KEY ?? '',
            smtpHost: emailSmtpHost,
            smtpPort: Number(firstNonEmpty(process.env.EMAIL_SMTP_PORT, process.env.MAIL_PORT, externalEmail?.smtpPort != null ? String(externalEmail.smtpPort) : undefined) || '587'),
            smtpSecure: (firstNonEmpty(process.env.EMAIL_SMTP_SECURE, process.env.MAIL_SECURE, externalEmail?.smtpSecure != null
        ...
        push: {
            ...
            iosSimulatorBundleId: process.env.PUSH_IOS_SIMULATOR_BUNDLE_ID ?? 'com.getnetbelay.bunnaBankMobile',
            apnsTeamId: process.env.APNS_TEAM_ID ?? '',
            apnsKeyId: process.env.APNS_KEY_ID ?? '',
            apnsBundleId: process.env.APNS_BUNDLE_ID ?? 'com.getnetbelay.bunnaBankMobile',
        ...
        telegram: {
            ...
            apiBase: process.env.TELEGRAM_BOT_API_BASE ??
                externalTelegram?.apiBase ??
                'https://api.telegram.org',
            forceTestChatId: process.env.TELEGRAM_FORCE_TEST_CHAT_ID ??
