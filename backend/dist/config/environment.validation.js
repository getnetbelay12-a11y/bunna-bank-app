...
function parseMongoConnectionDetails(uri) {
    ...
    if (!databaseName) {
        throw new Error(`MONGODB_URI must include a database name. Expected /${CURRENT_DATABASE_NAME}.`);
    }
    if (databaseName !== CURRENT_DATABASE_NAME) {
        throw new Error(`MONGODB_URI must target the ${CURRENT_DATABASE_NAME} database. Received ${databaseName}.`);
    }
    return {
        databaseName,
...
function validateEnvironment(config) {
    ...
    return {
        ...
        SMS_PROVIDER: requireEnumValue('SMS_PROVIDER', readString(config, 'SMS_PROVIDER', 'log'), VALID_NOTIFICATION_PROVIDERS),
        ...
        EMAIL_PROVIDER: requireEnumValue('EMAIL_PROVIDER', readString(config, 'EMAIL_PROVIDER', 'log'), VALID_EMAIL_PROVIDERS),
        ...
        PUSH_PROVIDER: requireEnumValue('PUSH_PROVIDER', readString(config, 'PUSH_PROVIDER', 'log'), VALID_NOTIFICATION_PROVIDERS),
        ...
        PUSH_IOS_SIMULATOR_BUNDLE_ID: readString(config, 'PUSH_IOS_SIMULATOR_BUNDLE_ID', 'com.getnetbelay.bunnaBankMobile'),
        APNS_TEAM_ID: readString(config, 'APNS_TEAM_ID'),
        APNS_KEY_ID: readString(config, 'APNS_KEY_ID'),
        APNS_BUNDLE_ID: readString(config, 'APNS_BUNDLE_ID', 'com.getnetbelay.bunnaBankMobile'),
        ...
        REPORTING_SNAPSHOT_LOOKBACK_DAYS: readNumber(config, 'REPORTING_SNAPSHOT_LOOKBACK_DAYS', 14, { min: 1 }),
        REPORTING_SNAPSHOT_LOCK_MINUTES: readNumber(config, 'REPORTING_SNAPSHOT_LOCK_MINUTES', 10, { min: 1 }),
    };
}
