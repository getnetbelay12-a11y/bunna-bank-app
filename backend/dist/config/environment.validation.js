...
exports.DEFAULT_MONGODB_URI = void 0;
exports.parseAllowedOrigins = parseAllowedOrigins;
exports.normalizeMongoUri = normalizeMongoUri;
exports.parseMongoConnectionDetails = parseMongoConnectionDetails;
...
exports.DEFAULT_MONGODB_URI = DEFAULT_MONGODB_URI;
...
function normalizeMongoUri(uri) {
    ...
    return trimmed.replace(new RegExp(`/${LEGACY_DATABASE_NAME}(?=\\?|$)`), `/${CURRENT_DATABASE_NAME}`);
}
function readString(config, key, defaultValue = '') {
...
function readNumber(config, key, defaultValue, { min } = {}) {
...
function requireSecret(config, key, minimumLength) {
...
function requireEnumValue(key, value, validValues) {
    if (!validValues.has(value)) {
        throw new Error(`Environment variable ${key} must be one of: ${Array.from(validValues).join(', ')}.`);
    }
    return value;
}
function requireNonEmptyWhenEnabled(enabled, provider, key, value, { providers } = {}) {
    if (!enabled) {
        return;
    }
...
function parseMongoConnectionDetails(uri) {
    ...
    if (!databaseName) {
        throw new Error('MONGODB_URI must include a database name. Expected /cbe_bank_app.');
    }
    if (databaseName !== CURRENT_DATABASE_NAME) {
        throw new Error(`MONGODB_URI must target the cbe_bank_app database. Received ${databaseName}.`);
    }
    return {
        databaseName,
...
function validateEnvironment(config) {
    ...
    const mongodbUri = normalizeMongoUri(readString(config, 'MONGODB_URI', DEFAULT_MONGODB_URI));
    ...
    const storageProvider = requireEnumValue('STORAGE_PROVIDER', readString(config, 'STORAGE_PROVIDER', 'local'), VALID_STORAGE_PROVIDERS);
    ...
    const smsProvider = requireEnumValue('SMS_PROVIDER', readString(config, 'SMS_PROVIDER', 'log'), VALID_NOTIFICATION_PROVIDERS);
    ...
    const emailProvider = requireEnumValue('EMAIL_PROVIDER', readString(config, 'EMAIL_PROVIDER', 'log'), VALID_EMAIL_PROVIDERS);
    const emailSender = readString(config, 'EMAIL_SENDER', 'notifications@bunna-bank.local');
    ...
    const pushProvider = requireEnumValue('PUSH_PROVIDER', readString(config, 'PUSH_PROVIDER', 'log'), VALID_NOTIFICATION_PROVIDERS);
    ...
    if (nodeEnv === 'production' && allowedOrigins.includes('*')) {
        throw new Error('CLIENT_APP_ORIGIN cannot use * in production. Set one or more explicit origins.');
    }
    if (storageProvider === 's3') {
        ...
        if (nodeEnv === 'production') {
            throw new Error('STORAGE_PROVIDER=s3 is not production-ready in this repo yet. Use local storage or implement the S3 upload client first.');
        }
    }
    requireNonEmptyWhenEnabled(smsEnabled, smsProvider, 'SMS_GENERIC_ENDPOINT', smsGenericEndpoint, { providers: ['generic_http'] });
    requireNonEmptyWhenEnabled(emailEnabled, emailProvider, 'EMAIL_GENERIC_ENDPOINT', emailGenericEndpoint, { providers: ['generic_http'] });
    requireNonEmptyWhenEnabled(pushEnabled, pushProvider, 'PUSH_GENERIC_ENDPOINT', pushGenericEndpoint, { providers: ['generic_http'] });
    requireNonEmptyWhenEnabled(emailEnabled, emailProvider, 'EMAIL_SMTP_HOST', emailSmtpHost, { providers: ['smtp'] });
    requireNonEmptyWhenEnabled(emailEnabled, emailProvider, 'EMAIL_SMTP_USER', emailSmtpUser, { providers: ['smtp'] });
    requireNonEmptyWhenEnabled(emailEnabled, emailProvider, 'EMAIL_SMTP_PASS', emailSmtpPass, { providers: ['smtp'] });
    requireNonEmptyWhenEnabled(emailEnabled, emailProvider, 'EMAIL_SENDER', emailSender, { providers: ['smtp'] });
    requireNonEmptyWhenEnabled(pushEnabled, pushProvider, 'FIREBASE_PROJECT_ID', firebaseProjectId, { providers: ['firebase'] });
    requireNonEmptyWhenEnabled(pushEnabled, pushProvider, 'FIREBASE_CLIENT_EMAIL', firebaseClientEmail, { providers: ['firebase'] });
    requireNonEmptyWhenEnabled(pushEnabled, pushProvider, 'FIREBASE_PRIVATE_KEY', firebasePrivateKey, { providers: ['firebase'] });
    if (nodeEnv === 'production' && pushEnabled && pushProvider === 'firebase') {
        throw new Error('PUSH_PROVIDER=firebase is not production-ready in this repo yet. Use log/generic_http or implement Firebase delivery first.');
    }
    return {
        ...config,
        NODE_ENV: nodeEnv,
        ...
        OTP_RESEND_COOLDOWN_SECONDS: readNumber(config, 'OTP_RESEND_COOLDOWN_SECONDS', 30, { min: 0 }),
